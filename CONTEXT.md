...
**Bestellung**:
Ein Kundenauftrag mit Positionen. Kein Lagerobjekt und keine Werkstattzeile. Erster Eingang: **manuelle Schnellerfassung** (**Datum + Uhrzeit**, beim Anlegen auf jetzt vorbelegt, änderbar; mindestens eine Position; Kundenname und Nummer optional). Externe Nummern und Zeitpunkt später aus Shopify/Etsy. **Herkunft** folgt dem Eingang (nicht frei umlabeln). Status: **offen** (offene Todos), **versandbereit** (alle Todos erledigt, Positionen ohne Todo gelten als erfüllt), **versendet**. **Versendet** zuerst manuell an der Bestellung; später zusätzlich aus Shopify fulfilled. Bleibt sichtbar bis versendet.
_Avoid_: Todo als Synonym für den Shop-Auftrag; Bestellung = einzelne Position; Shop-API/CSV als Voraussetzung für die erste Todo-Liste; Pflicht-Kundenname oder Pflicht-Shopnummer zum Anlegen; nach Werkstatt-Ende ausblenden; Versenden = Fertigen/Zusammenstellen; Versand-Todo; Versendet nur über Shop ohne manuellen Weg; nur Datum ohne Uhrzeit; Herkunft unabhängig vom Import setzen

**Herkunft** (Bestellung):
Woher der Auftrag in die App kam: **Shopify**, **Etsy** oder **Manuell**. Folgt aus dem Eingang (Shop-Import bzw. Schnellerfassung). Gruppierung der Listen danach kommt später mit den Importen.
_Avoid_: Channel, Kanal als UI-Label; Shop-Name als Herkunft; Herkunft nachträglich frei wählen; Herkunft nur als Filter ohne die drei Werte

**Aktuelle Bestellungen**:
Bestellungen mit Status **offen** oder **versandbereit** (nicht versendet). Eigene Liste **oben** auf der Bestellseite (darunter Schnellerfassung, versendete ausklappbar) und in der Übersicht.
...
**Filter-Leiste**:
Unter der Hauptnavigation, **nur** auf Materialien und Produkte (nicht auf der Übersicht). Eine Zeile (notfalls zwei): **Farbe** immer **mit Medium**, aufklappbar — mehrere Medien gleichzeitig möglich; nach Wahl eines Mediums dessen Farben als Kästchen (Hover = Name); keine Farbe = alle Farben dieses Mediums; eine oder mehrere Farben = nur die. Wählen eines Mediums setzt zusätzlich **Familie** und **Tag** mit **gleichem Namen**, falls sie existieren. Medium abwählen entfernt die gleichnamige Familie und das gleichnamige Tag wieder (auch wenn sie schon vorher angehakt waren). **Familie** und **Tag** als Mehrfach-Auswahl; plus **Freitext**. Innerhalb einer Filterart **ODER**, zwischen den Arten **UND**. **Ein gemeinsamer Filterstand** über Materialien und Produkte; auf der jeweiligen Seite gelten Filter für **Hauptliste und Extra-Blöcke**. Sichtbarer Hinweis, wenn Filter aktiv sind, und eine Aktion **alle Filter entfernen**. Nicht auf Übersicht/Bestellungen/Todos/Sets/Import.
_Avoid_: Filter als dominante linke Spalte; Filter verstecken hinter Icon als einzigem Zugang; Filterleiste auf der Übersicht oder auf Bestellungen; Filter ohne Hinweis/Reset bei aktivem Stand; flache Farbliste über alle Medien; Farbe ohne Medium im Filter; nur ein Medium gleichzeitig im Filter; Medium-Wahl setzt ungefähr passende Familien oder Tags ohne Namensgleichheit; Familie = Tag; gleichnamige Familie/Tag bleiben nach Medium-Abwahl automatisch stehen; Extra-Blöcke auf Material/Produkt ohne Filter
