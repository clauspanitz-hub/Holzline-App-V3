...
**Bestellposition**:
Eine Zeile einer Bestellung: zugeordnetes **Produkt**, **Set-Variante** (sobald Sets im Bestellfluss), oder **unzugeordnet** (Freitext), bis ein **Artikel** verknüpft ist. Erfassung kompakt: Produktwahl, Menge, **Entfernen**. Freitext nur sichtbar, wenn kein Produkt gewählt ist — dann nicht nötig, sobald ein Produkt gesetzt ist. Unzugeordnet erzeugt ein Todo **Artikel anlegen** (wählen: Produkt oder Material; bei Bestellung Standard Produkt) — erst sobald die Bestellung **offen** ist. Nach dem Anlegen: Position **verknüpfen**, Todos neu bewerten. Solange **zur Prüfung**: Zuordnung und Menge korrigierbar, ohne Todo. Shop-Eingang ordnet eine Zeile über **SKU**, sonst **Shop-Zuordnung**, sonst Name/Farbe; kein eindeutiger Treffer → **unzugeordnet** (Freitext = Shop-Titel). Nach dem Abruf optional ein Gemini-**Vorschlag** an der Zeile (sichtbar, nicht vorausgewählt; **Übernehmen** bestätigt); gilt auch für schon vorhandene unzugeordnete Prüfungszeilen ohne Vorschlag; ohne Schlüssel entfällt er. Bestätigen speichert die Shop-Zuordnung; andere unzugeordnete Prüfungszeilen derselben Identität ziehen mit, Abnicken bleibt.
_Avoid_: Pflicht, dass der Artikel vor der Bestellung existiert; Freitext ohne Folge-Todo; Freitext-Pflicht neben gewähltem Produkt; Set als einzige Positionsart; Anlegen ohne Verknüpfung an die Position; „Zeile weg“ als Label; Todo Artikel anlegen schon zur Prüfung; jede Shop-Zeile bewusst ohne Match; Set-Variante als Match in diesem Schnitt; Gemini als Gedächtnis; stilles Öffnen durch KI; Gemini bei jeder Tastatureingabe; Vorschlag schon im Auswahlfeld als wäre zugeordnet
