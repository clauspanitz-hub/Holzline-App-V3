...
**Produktfamilie**:
Katalogknoten (Elternfamilie oder eine **Unterfamilie**) aus dem **Produktfamilien**-Katalog, unter dem zusammengehörige Produkte gruppiert werden (z. B. Eltern `Ziffern`, Unterfamilie `Einzeln`). Am Produkt hängt nur **eine** Referenz (Eltern *oder* Unter). Bei der Serienanlage: Find-or-create **Elternfamilie** aus dem **Basisnamen**. Pflege im Tab **Kataloge**; am Produkt nur Auswahl. In der UI: Filter nach Elternfamilie (inkl. Unterfamilien-Artikel) und einklappbare Gruppenköpfe Eltern → optional Unterköpfe, **Standard geschlossen**. Dieselbe Gruppierung in **Auswahllisten**. Ohne Familie = „Ohne Familie“. Beim Einführen: einmaliger Vorschlag aus Name ohne Farb-Suffix (→ Elternfamilie), Bestätigung nötig. Unabhängig von Farbe/Tag; nicht der Produktname selbst.
_Avoid_: Nur implizite Gruppierung über Namensähnlichkeit ohne Feld; Familie = Tag; stille automatische Befüllung ohne Nachfrage; Freitext-Familie am Artikel statt Katalog; gemeinsame Liste mit Materialfamilien; Familien in Listen standardmäßig aufgeklappt; flache Selects wo Familien existieren; mehr als eine Unterebene

**Materialfamilie**:
Analog zur **Produktfamilie**, aber eigener Katalog (nicht dieselbe Liste). Serienanlage: Find-or-create Elternfamilie aus Basisname. Pflege im Tab **Kataloge**; am Material nur Auswahl. Filter Eltern inkl. Unterfamilien-Artikel; Listen: Elternkopf → optional Unterköpfe. Ohne Familie = „Ohne Familie“.
_Avoid_: Gemeinsame Familien-Liste mit Produkten; Familie = Tag; Freitext am Artikel; nur Gruppierung über Medium/Farbe ohne Feld; Familien standardmäßig aufgeklappt; mehr als eine Unterebene
