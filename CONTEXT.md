...
JSON-Datei für **Inventar** (Kataloge, Materialien inkl. Bezugsquellen/Shops, Produkte, Bestände, Sets u. a.) zum Herunterladen und Wiederherstellen — nicht Bestellungen, Todos, Shop-Zuordnungen, Tageslage oder Benutzer (dafür Host-Backup der SQLite). Bewegungs-Historie optional per Checkbox (Standard: aus). Beim Import wählt der Nutzer: **Ersetzen** (bestehende Daten werden geleert/ersetzt) oder **Zusammenführen** (Match über Name bzw. SKU/Handle/Farbe+Medium; fehlende anlegen, Stammdaten aktualisieren; Bestände je Standort beim Match **setzen**, nicht addieren) — jeweils mit Bestätigung. Kein Ersatz für Host-Backups der SQLite-Datei.
